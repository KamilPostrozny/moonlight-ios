...
- (void)main {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        VideoDecoderRenderer* renderer = [[VideoDecoderRenderer alloc] initWithView:self->_renderView callbacks:self->_callbacks streamAspectRatio:(float)self->_config.width / (float)self->_config.height framePacingMode:self->_config.framePacingMode];
...
- (NSString*) getStatsOverlayText {
    ...
    return [NSString stringWithFormat:@"Video stream: %dx%d %.2f FPS (Codec: %@)\nFrames dropped by your network connection: %.2f%%\nAverage network latency: %@%@\n%@",
            ...
            latencyString,
            hostProcessingString,
            [_connection getFramePacingStatsText]];
}
